...
import { EMPLOYEE_CATEGORY_OPTIONS } from '../../utils/employeeListQuery';

const MENU_MIN_WIDTH = 168;

const EmployeeCategoryFilterButton = ({
  value = '',
  onChange,
  disabled = false,
  compact = false,
  ...
  const selectedLabel = EMPLOYEE_CATEGORY_OPTIONS.find((opt) => opt.value === value)?.label;
  ...
  const filterTitle = value
    ? `Category: ${selectedLabel}. Click to change.`
    : 'Filter by category';
  ...
  return (
    <>
      <div className={`relative ${compact ? 'inline-flex max-w-full' : 'shrink-0'}`} ref={rootRef}>
        ...
        >
          <span className={compact ? 'truncate' : undefined}>Category</span>
...
export default EmployeeCategoryFilterButton;
