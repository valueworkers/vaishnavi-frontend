...
import { EMPLOYEE_TYPE_OPTIONS } from '../../utils/employeeListQuery';

const MENU_MIN_WIDTH = 140;

const EmployeeTypeFilterButton = ({
  value = '',
  onChange,
  disabled = false,
  compact = false,
  ...
  const selectedLabel = EMPLOYEE_TYPE_OPTIONS.find((opt) => opt.value === value)?.label;
  ...
  const filterTitle = value
    ? `Emp Role: ${selectedLabel}. Click to change.`
    : 'Filter by emp role';
  ...
  return (
    <>
      <div className={`relative ${compact ? 'inline-flex max-w-full' : 'shrink-0'}`} ref={rootRef}>
        ...
        >
          <span className={compact ? 'truncate' : undefined}>Emp Role</span>
...
export default EmployeeTypeFilterButton;
